<template>
  <div class="hello">
    ...
    table组件 + 假分页 <el-button size="mini" @click="other_table = !other_table">{{ other_table ? '切换为全列表格' :
      '切换为表格2' }}</el-button>
    ...
    <span style="
      ...
      ">
      <el-button v-if="mutiSelected.length === 0" size="mini" type="danger" @click="allDelete">批量删除</el-button>
      <el-button v-else @click="handleAllDelete" size="mini" type="danger">确认删除</el-button>
    </span>
    <span v-if="mutiSelected.length > 0" style="
          position: relative;
          left: 20px;
          white-space: nowrap;
          overflow: hidden;
    ...
    <div v-if="card_listData.length > 0">
      <div v-for="(text, index) of card_listData" :key="'lib' + index">
        ...
        -->
        <CP_MyCard :data="text" :footerHidden="false" :deleteHidden="text._id === cardId ? deleteHidden : true"
          :cancelSelected="cancelSelected" :multiDelete="multiDelete" @getHidden="getHidden"
          @deleteSingleData="deleteSingleData(text._id)" @getId="getId" :subscribe="text.subscribe" :unfit="text.unfit">
          {{ text }}
          ...
          <template #button_option="{ text }"> <!-- 接收、 插入 -->
            ...
            <el-button size="mini" type="primary"
              @click.stop="$router.push({ path: `/cp_myeditpage`, name: 'cp_myeditpage', params: text, query: { id: text._id } })">进入编辑</el-button>
    ...
    <!-- FUNCTION: Tree -->
    Tree组件
    <CP_MyTree v-if="treeData.length" style="margin-top: 5px" :data="treeData" :currTag="currentNode" :show-menu="true"
      :expanding="true" :context-menu-items="menuItems" @onCurrChange="handleNode" @contextMenuClick="contextMenuClick">
    </CP_MyTree>
    <hr>
    <!-- FUNCTION: 筛选 -->
    筛选组件
    <CP_MyFilter :data="filterData" :total="total" cate="文章" :largeTitle="false" :noLanguage="false"
      :language="language" @handleClick="handleFilterClick">
    </CP_MyFilter>
    <hr>
    ...
    <div>
      ...
      <draggable v-model="hobbies" @start="moveTagStart" @end="moveTagEnd" filter=".unDrag">
        ...
        <el-tag v-for="(item, index) in hobbies" closable @close="handleDelete(item)"
          style="margin-left:10px; cursor: pointer;" @click="handleClickTag(index)"
          :class="{ unDrag: openDrag ? item === '篮球' : true }" :style="{
          ...
          <el-input v-if="editing[item]" autofocus v-model="hobbyInput" :key="item" @blur="closeInput(item)" size='mini'
            style="width:60px;font-size:12px"></el-input>
        </el-tag>
      </draggable>
    </div>
    <hr>
    ...
    <el-dialog title="json变成tree结构" :visible.sync="packageToolVisible" class="package-tool" width="600px" append-to-body
      :close-on-click-modal="false" :destroy-on-close="true">
      <PackagingTool type="pptx" title="json大文件的名字。。。。。" :project-json="projectJson"
    ...
    <el-dialog title="json变成tree结构 + 可拖拽" :visible.sync="packageToolDragVisible" class="package-tool" width="600px"
      append-to-body :close-on-click-modal="false" :destroy-on-close="true">
      <PackagingToolDrag type="pptx" title="json大文件的名字。。。。。" :project-json="projectJson"
        :zip_file_info="{ 'chapter': 1, 'hidden_spilt_title': true, 'add_prefix_number': true }"
    ...
    <el-dialog :visible.sync="catalogEditorVisible" :append-to-body="true" :show-close="false" width="600px"
    ...
    <el-input placeholder="请输入内容" prefix-icon="el-icon-search" style="width: 200px" size="mini" v-model="lodash_search"
      @input="lodashSearchList" />
    <hr>
    ...
    <!-- 
      ...
      -->
    <Upload v-model="uploading" class="min-400" :cancel_load="cancel_load" :multiple="false" @upload="uploadHandler"
      @fileURL="getURL" @process="getProcess" @getFileName="getFileName" :allowToUpload="allowToUpload" />
    <hr>
    ...
    <el-dialog title="添加文章标签" :visible.sync="dialog_visible" width="30%" append-to-body :close-on-click-modal="false">
      <span>这是一段信息</span>
    ...
    <br><br>
    <!-- FUNCTION: v-for -->
    <div v-for="(item, index) in inputContent" v-bind:key="index" style="
          display: flex;
      ...
      <i class="el-icon-circle-plus" @click="addContent" style="font-size: 18px; cursor: pointer;"></i>
    </div>
    <hr>
    <br><br>
    <!-- FUNCTION: 下拉框多选 -->
    ...
    <!-- FUNCTION: el自带动画 -->
    el自带动画
    <el-button @click="showTransition = !showTransition">Click Me</el-button>

    <div style="display: flex; margin-top: 20px; height: 100px;">
      <transition name="el-fade-in-linear">
        <div v-show="showTransition" class="transition-box">.el-fade-in-linear</div>
      </transition>
      <transition name="el-fade-in">
        <div v-show="showTransition" class="transition-box">.el-fade-in</div>
      </transition>
    </div>
    <br><br>
    <br><br>
...
export default {
  ...
  methods: {
    ...
    handleConfirm () {
      ...
      //   cancelButtonText: '取消',
      //   showInput: true,
      //   inputPlaceholder: '请输入书名验证',
      //   beforeClose: (action, instance, done) => {
      //     if (action === 'confirm') {
      //       const isValid = instance.inputValue.trim() !== this.book.name;
      //       if (isValid) {
      //         this.$message.error('书名输入错误, 请重新输入');
      //         done(false);
      //         return;
      //       }
      //       this.$message.success('书名匹配,可以删除!');
      //       done();
      //       this.handleRemove();
      //     } else {
      //       done();
