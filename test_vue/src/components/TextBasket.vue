<template>
  <div ref="textBasket" id="textBasket" class="text-basket-container m-b-10"
          @mousedown="dragMe"
  >
      <!-- 徽章 -->
  ...
  methods:{
    ...
    handleOpened(){
      if (this.$refs.textBasket) {
        // (this.$refs.textBasket as any).style.right = '30%';     // TS写法
        this.$refs.textBasket.style.right = '30%';
      } 
      // 通知父组件
      this.$emit('open');  
    },
    ...
    handleClose(){
      if (this.$refs.textBasket) {
        // (this.$refs.textBasket as any).style.right = '0%';     // TS写法
        this.$refs.textBasket.style.right = '0%';
      }
      // 通知父组件
      this.$emit('close');
    }
