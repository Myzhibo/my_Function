<template>
  <div class="my-tree" style="height: 100%; overflow-y: auto;">
    <RecycleScroller
      class="scroller"
      :items="list"
      :item-size="40"
    ...
    >
      <div
        ...
        :class="{
          ...
          'item-style-active': activeNodeId ?
                               activeNodeId === item.node_id :
                               (checkedKeys ? checkedKeys.includes(item.node_id) : false),
          'disable-node': disableNodeKeys.includes(item.node_id) || item._parent.node_type !== 'chapter'
        }"
        :style="{backgroundColor: ifOnlyLeafNode ? (hoverNoLeafNodeId === item.node_id ? 'aliceblue' : '') : (hoverNodeId === item.node_id ? 'aliceblue' : ''), 
                  color: ifOnlyLeafNode ? (hoverNoLeafNodeId === item.node_id ? '#409eff' : '')  : (hoverNodeId === item.node_id ? '#409eff' : ''),
                  opacity: ifOnlyLeafNode ? (hoverNoLeafNodeId === item.node_id ? 0.8 : '')  : (hoverNodeId === item.node_id ? 0.8 : ''),
                  cursor: ifOnlyLeafNode ? (hoverNoLeafNodeId === item.node_id ? 'pointer' : '')  : (hoverNodeId === item.node_id ? 'pointer' : '')
                }"
        >
        <div
        ...
        >
          <div class="item-padding" :style="paddingLeft(item)">
            <div class="item-inner flex-center" >
                <!-- :style="{cursor : (ifOnlyLeafNode && item.children.length ? '' : 'pointer')}" -->
...
export default {
  ...
  methods: {
    ...
    nodeClick (item) {
      if (this.ifOnlyLeafNode && item.children.length) {
        // 如果只允许叶子节点可以被点击，那么直接 return 出去
        return;
      }
      ...
      if (item.node_type === 'chapter' || item._parent.node_type === 'chapter') {
        this.$emit('nodeClick', this.riginalDataMap.get(item.node_id));
      }
    },
    checkChange() {
      this.selectList = this.flattenJson.filter((item) => item._checked);
      this.$emit('checkChange', this.selectList.map(item => this.riginalDataMap.get(item.node_id)));
    },
    html2text(html) {
    ...
    nodeEvent(item, eventName) {
      if (eventName === 'mouseleave') {
        this.hoverNodeId = '';
        this.hoverNoLeafNodeId = '';
        return;
      } else if (eventName === 'mouseover') {
        this.$emit('hoverNode', item);
        if (this.ifOnlyLeafNode && !item.children.length) {
          this.hoverNoLeafNodeId = item.node_id;
        }
    ...
    handleDelete(type) {
      ...
      if (type === 'children') {
        ...
        const children = this.flattenJson.filter(node => node._path.some(item => item.node_id === this.currNode.node_id));
        if (this.currNode.node_type === 'chapter' && children.length) return this.$alert('该标题含有内容，请将内容移出后重试', '', {confirmButtonText: '我知道了'});
        this.currNode._parent.children.splice(index, 1);
        // 更新原数据
        this.riginalDataMap.get(this.currNode._parent_id).children.splice(index, 1);
        this.riginalDataMap.get(this.currNode._parent_id).children.forEach((item, index) => {
