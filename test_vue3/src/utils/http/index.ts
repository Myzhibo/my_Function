...
class PureHttp {
  ...
  public request<T>(
    ...
    // @tips：根据域名判断使用什么 host，目前只支持万维图库网。
    if (window.location.href.includes('wwtk') && !/http/.test(url)) {
      url = url.replace(/^\//, '');
  ...
  /** 单独抽离的delete工具函数 */
  public delete<T, P>(url: string, params?: T, config?: PureHttpRequestConfig): Promise<P> {
    return this.request<P>('delete', url, params, config);
  }
  /** 单独抽离的put工具函数 */
  public put<T, P>(url: string, params?: T, config?: PureHttpRequestConfig): Promise<P> {
